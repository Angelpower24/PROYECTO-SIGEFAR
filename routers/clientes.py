...
from dao.cliente_dao import ClienteDAO, ClienteNoEncontradoError, DNIDuplicadoError, ClienteConVentasError
...
def crear_cliente(datos: ClienteCrear):
    try:
        c = dao.insertar(Cliente(datos.nomb_cli, datos.ape_cli, datos.dni, datos.telefono))
        return c.to_dict()
...
def actualizar_cliente(cliente_id: int, datos: ClienteActualizar):
    try:
        c = dao.actualizar(cliente_id, datos.nomb_cli, datos.ape_cli, datos.telefono)
        return c.to_dict()
